...
export const apiGetAllProfiles = createAsyncThunk(
  ...
  async (_, thunkApi) => {
    try {
      ...
      return data; //те, що повертається з Санки потрапляє в action.payload в статусі fulfilled
    } catch (error) {
      return thunkApi.rejectWithValue(error.message); //те, що повертається з Санки потрапляє в action
    }
  }
);
  ...
  async (profileId, thunkApi) => {
    try {
      const { data } = await axios.delete(
        `https://678aa23cdd587da7ac2aeb83.mockapi.io/api/v1/profile/${profileId}`
      );
      return data; //те, що повертається з Санки потрапляє в action.payload в статусі fulfilled
    } catch (error) {
      return thunkApi.rejectWithValue(error.message); //те, що повертається з Санки потрапляє в action
